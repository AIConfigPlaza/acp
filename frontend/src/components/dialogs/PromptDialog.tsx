...
export function PromptDialog({ open, onOpenChange, prompt, onSave }: PromptDialogProps) {
  ...
  useEffect(() => {
    if (prompt) {
      ...
      setContentMd(prompt.content_md);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-card border-border max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {prompt ? t("dialog_edit_prompt") : t("dialog_new_prompt")}
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder={language === "zh" ? "你是一名代码审查专家...\n\n{{code}}" : "You are a code review expert...\n\n{{code}}"}
              className="bg-surface-2 font-mono min-h-[250px]"
            />
          </div>
